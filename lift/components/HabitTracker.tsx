...
/** Streak = consecutive calendar days with the habit done (any dayIndex entry for that date counts) */
function getStreak(log: HabitEntry[], key: 'walk' | 'creatine'): number {
  let streak = 0;
  const cursor = new Date(todayISO());
  for (let i = 0; i < 365; i++) {
    const dateStr = cursor.toISOString().split('T')[0];
    const done = log.some((e) => e.date === dateStr && e[key]);
    if (done) {
      streak++;
    } else if (i > 0) {
      break; // gap found — stop
    }
    // i === 0 and not done: today not checked yet, look back from yesterday
...
export default function HabitTracker({ profile, onUpdate }: HabitTrackerProps) {
  ...
  // Each workout day gets its own entry, keyed by date + dayIndex
  const todayEntry: HabitEntry = profile.habitLog.find(
    (e) => e.date === today && (e.dayIndex ?? 0) === dayIndex,
  ) ?? { date: today, dayIndex, walk: false, creatine: false };

  const isLate = new Date().getHours() >= 11 && (!todayEntry.walk || !todayEntry.creatine);

  const handleToggle = (key: 'walk' | 'creatine') => {
    const updated: HabitEntry = { ...todayEntry, [key]: !todayEntry[key] };
    const log = profile.habitLog.filter(
      (e) => !(e.date === today && (e.dayIndex ?? 0) === dayIndex),
    );
    log.push(updated);
  ...
  const walkStreak = getStreak(profile.habitLog, 'walk');
  const creatineStreak = getStreak(profile.habitLog, 'creatine');
    ...
    <div className="card">
      ...
      {isLate && (
        <div style={{
          ...
          display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', color: '#ff8888',
        }}>
          <span>⚠</span><span>It&apos;s past 11 AM — don&apos;t forget your habits!</span>
        </div>
      )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
              ...
              {habit.streak > 0 && (
                <span style={{
                  ...
                  padding: '0.1rem 0.4rem', borderRadius: '4px', fontFamily: 'monospace',
                }}>
                  🔥 {habit.streak}
                </span>
              )}
