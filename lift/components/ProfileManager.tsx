    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      {profiles.map((profile) => (
        <div
          ...
          className="card"
          style={{
            borderColor: profile.id === activeProfileId ? '#e8ff47' : '#222',
          }}
        >
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                {profile.id === activeProfileId && (
                  <span style={{ fontSize: '0.7rem', background: '#e8ff47', color: '#0a0a0a', padding: '0.1rem 0.4rem', borderRadius: '4px', fontWeight: 700 }}>
                    ACTIVE
                  </span>
                )}
              </div>
            ...
            {LIFTS.map((lift) => (
              <div key={lift} style={{ background: '#1a1a1a', borderRadius: '6px', padding: '0.6rem', textAlign: 'center' }}>
                <div style={{ color: '#888', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.2rem' }}>
                  {lift === 'press' ? 'OHP' : lift.slice(0, 2).toUpperCase() + lift.slice(2, lift.length > 5 ? 3 : undefined)}
                </div>
